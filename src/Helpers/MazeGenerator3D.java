...
public class MazeGenerator3D {
    ...
    public int[][][] generate(MazeGenerator.FinishMode mode) {
        ...
        entryX = entryY = new int[floorCount];
        ...
        for (int f = 0; f < floorCount - 1; f++) {
            int[] stairPos = pickStairPosition(floors[f], entryX[f], entryY[f]);
    ...
    //region Helpers
    private int[] pickStairPosition(int[][] floorMaze, int fromX, int fromY) {
        List<int[]> reachable = bfsOpenCells(floorMaze, fromX, fromY);
        reachable.removeIf(cell -> cell[0] == fromX && cell[1] == fromY);
        if (reachable.isEmpty()) return new int[]{fromX, fromY};
    ...
    private void placeFinish(MazeGenerator.FinishMode mode) {
        ...
        switch (mode) {
            ...
            case CENTER -> {
                ...
                while (floorMaze[finishY][finishX] == WALL) {
                    finishX += random.nextBoolean() ? 1 : -1;
                    finishY += random.nextBoolean() ? 1 : -1;
                    finishX = Math.clamp(finishX, 1, mazeWidth - 2);
                    finishY = Math.clamp(finishY, 1, mazeHeight - 2);
                }
            }
            case RANDOM_EDGE -> {
